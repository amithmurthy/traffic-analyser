...
from utils import mac_addr, inet_to_str, pickle_obj, unpickle_obj, jsonpickle_obj
import itertools
import math
...
def read_pkt_hdrs(network_inst, pkts):
    ...
    for t, pkt in pkts:
        ...
        if isinstance(ip_pkt.data, TCP) or isinstance(ip_pkt.data, UDP):
            ...
            flow_tuple = (
            pkt_struct['ip_src'], pkt_struct['ip_dst'], pkt_struct['sport'], pkt_struct['dport'], protocol)
        else:
...
def validate_network_obj():
    ...
    #         # If the timestamp of pkt_structs in Node.traffic is different => multiprocessing has
...
def get_node_table(network):
    ...
    for node in network.GraphNetwork.nodes:
        node_data = {'mac_addr': node.mac_addr, 'uplink_total': node.uplink_total,
                     'downlink_total': node.downlink_total}
...
def parse_sequentially(file_path):
    ...
    with open(file_path, 'rb') as f:
        ...
        for t, pkt in pkts:
            ...
            progress = math.floor(pkts_read / pkt_volume * 100)
            # if progress % 5 == 0:
            #     print(progress)
        end = time.time()
...
if __name__ == "__main__":
    ...
    # pickle_obj(name='network', obj=network_inst, isNetworkProxy=True)goo
